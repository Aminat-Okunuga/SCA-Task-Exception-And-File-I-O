...
try {
    ...
    $dest = "C:/wamp/www/SCA-Task/ProductManagementSystem/Product/pictures/" . $_FILES['picture']['name'];
...
//exit();
    ...
    if (!move_uploaded_file($_FILES['picture']['tmp_name'], $dest)) {
        ...
        $categoryError = Validator::validateText2('Category', $category, 30);
        // TODO: Check if category exists
        if ($categoryError != null) {
            throw new \Exception($categoryError);
        }

        $product = new Entity\Product($name, $description, $price, $brand, $color, $picture, $seller, $category, 0);
